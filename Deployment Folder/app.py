...
[data-testid="stSidebar"] .stMarkdown p {
    color: var(--muted) !important; font-size: 0.72rem !important; letter-spacing: .12em !important;
}
...
.stButton > button {
    background: var(--accent) !important; color: #111318 !important;
...
.stDownloadButton > button {
    background: var(--bg-card) !important; color: var(--text) !important;
    border: 1px solid var(--border) !important; border-radius: 14px !important; font-weight: 600 !important;
}

.stTabs [data-baseweb="tab-list"] {
    gap: 8px !important; background: transparent !important; border-bottom: 1px solid var(--border) !important;
}
.stTabs [data-baseweb="tab"] {
    background: transparent !important; border-radius: 10px 10px 0 0 !important;
    color: var(--muted) !important; font-weight: 600 !important; padding: .5rem 1rem !important;
}
...
.streamlit-expanderContent {
    background: #0f1117 !important; border: 1px solid var(--border) !important;
...
.ai-card {
    ...
    border-radius: 20px; padding: 1.5rem; transition: border-color .2s, transform .2s;
}
.ai-card:hover { border-color: rgba(212,248,112,.4); transform: translateY(-2px); }

.dest-card {
    background: var(--bg-card); border: 1px solid var(--border);
    border-radius: 20px; overflow: hidden; transition: border-color .25s, transform .25s;
}
...
.tag {
    ...
    border-radius: 8px; padding: 2px 10px; font-size: .78rem; font-weight: 700; letter-spacing: .04em;
}
.tag-dark {
    display: inline-block; background: rgba(255,255,255,.06); color: var(--muted);
    border-radius: 8px; padding: 2px 10px; font-size: .75rem; font-weight: 600;
}
.section-title {
...
.chat-user { display: flex; justify-content: flex-end; margin: .5rem 0; }
.chat-bot  { display: flex; justify-content: flex-start; margin: .5rem 0; }
.bubble-user {
    background: var(--accent); color: #111318; border-radius: 18px 18px 4px 18px;
    padding: .75rem 1.1rem; max-width: 75%; font-weight: 500; font-size: .9rem; line-height: 1.5;
}
.bubble-bot {
...
.model-badge {
    ...
    background: rgba(212,248,112,.1); border: 1px solid rgba(212,248,112,.3);
    border-radius: 8px; padding: 3px 10px; font-size: .72rem;
    font-weight: 700; color: #d4f870; letter-spacing: .04em;
}
...
# ═══════════════════════════════════════════════════════════════
#  GEMINI CLIENT
#  Key loaded exclusively from:
#    1. st.secrets["GEMINI_API_KEY"]  (.streamlit/secrets.toml)
#    2. GEMINI_API_KEY environment variable (local dev fallback)
# ═══════════════════════════════════════════════════════════════
...
def _call(prompt: str | list, system: str = None,
          json_out: bool = False, temp: float = 0.8) -> str | None:
    """Generic Gemini call — always uses GEMINI_MODEL. Returns text or None."""
    client = get_client()
    if not client:
        return None
...
# ═══════════════════════════════════════════════════════════════
#  AI FUNCTIONS  (all text calls route through _call → GEMINI_MODEL)
# ═══════════════════════════════════════════════════════════════
def ai_weather_suggestion(city: str, temp: int, cond: str) -> str:
    r = _call(
        f"City: {city}, Temp: {temp}°C, Condition: {cond}. "
        "Give 1-2 sentence activity suggestion for a traveler.",
        temp=0.6,
    )
...
def ai_generate_itinerary(prefs: dict, days: int) -> list:
    ...
    raw = _call(prompt, json_out=True, temp=0.85)
    if not raw:
        return []
    try:
...
def ai_refine_itinerary(plan: list, request: str) -> list:
    ...
    raw = _call(prompt, json_out=True, temp=0.75)
    if not raw:
        return plan
    try:
        return json.loads(re.sub(r"```json|```", "", raw).strip())
...
def ai_translate(text: str, lang: str) -> str:
    r = _call(f'Translate to {lang}: "{text}". Return only the translation.')
    return r or text
...
def ai_translate_itinerary(plan: list, lang: str) -> list:
    ...
    for day in plan:
        out.append({**day,
                    "title":       ai_translate(day["title"], lang),
                    "description": ai_translate(day["description"], lang)})
    return out
    ...
    try:
        chat = client.chats.create(
            ...
            config=genai_types.GenerateContentConfig(
                system_instruction=ARIA_SYSTEM, temperature=0.82),
        )
...
def ai_packing_list(dest: str, days: int, season: str, interests: list) -> list:
    ...
    raw = _call(prompt, json_out=True, temp=0.7)
    if not raw:
        return []
    try:
        return json.loads(re.sub(r"```json|```", "", raw).strip())
...
def ai_journal_expand(notes: str, dest: str) -> str:
    ...
    ) or "Could not expand your journal."


def ai_generate_image(prompt: str, aspect_ratio: str = "16:9") -> bytes | None:
    """Imagen 3 — dedicated image model, separate from GEMINI_MODEL."""
    client = get_client()
    if not client:
        return None
    try:
...
def ai_analyze_image(img_bytes: bytes, mime: str, user_prompt: str) -> str:
    """Vision — uses GEMINI_MODEL (2.5 Pro is natively multimodal)."""
    client = get_client()
    if not client:
...
def ai_generate_video(prompt: str) -> str | None:
    """Veo 2 — dedicated video model, separate from GEMINI_MODEL."""
    client = get_client()
    if not client:
        return None
    import time
    try:
        op = client.models.generate_videos(
            ...
            config=genai_types.GenerateVideosConfig(
                number_of_videos=1, duration_seconds=5, aspect_ratio="16:9"),
        )
        for _ in range(30):
            if op.done:
                break
...
def make_pdf(itinerary: list, destination: str) -> bytes | None:
    ...
    try:
        class TripPDF(FPDF):
            def header(self):
                self.set_fill_color(212, 248, 112); self.rect(0, 0, 210, 18, "F")
                self.set_font("Helvetica", "B", 13); self.set_text_color(17, 19, 24)
        ...
        pdf.set_font("Helvetica", "B", 22); pdf.set_text_color(17, 19, 24)
        pdf.cell(0, 12, destination, ln=True)
        pdf.set_font("Helvetica", "", 11); pdf.set_text_color(100, 110, 130)
        pdf.cell(0, 8, f"{len(itinerary)}-Day Cultural Experience  •  {datetime.now().strftime('%B %Y')}", ln=True)
        pdf.ln(6)
        for day in itinerary:
            pdf.set_fill_color(28, 31, 40); pdf.set_draw_color(42, 47, 61)
            pdf.set_font("Helvetica", "B", 12); pdf.set_text_color(212, 248, 112)
            pdf.cell(0, 9, f"Day {day.get('day','')}  -  {day.get('title','')}", ln=True, fill=True)
            pdf.set_font("Helvetica", "", 10); pdf.set_text_color(200, 205, 215)
            pdf.multi_cell(0, 6, day.get("description", ""), align="L")
            pdf.set_font("Helvetica", "I", 9); pdf.set_text_color(107, 114, 128)
            pdf.cell(0, 7, f"  {day.get('location','')}   {day.get('weather','')}", ln=True)
            pdf.ln(4)
...
def make_tts(text: str, lang_code: str = "en") -> bytes | None:
    ...
    try:
        ...
        st.warning(f"TTS error: {e}"); return None


def md_render(text: str):
    html = md_lib.markdown(text, extensions=["tables", "nl2br"]) if MD_OK else text.replace("\n", "<br>")
    st.markdown(f"""
    <div style='color:#d1d5db;line-height:1.7;font-size:.9rem;
                padding:1rem;background:#0f1117;border-radius:12px;border:1px solid #2a2f3d;'>
        {html}
...
def sidebar():
    with st.sidebar:
        ...
        # ── Gemini connection status + active model badge ──
        if get_client():
            st.markdown(f"""
            ...
            <div style='text-align:center;margin-bottom:1rem;'>
                <span class='model-badge'>✦ {GEMINI_MODEL}</span>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style='display:flex;align-items:center;gap:.6rem;
        ...
        if not st.session_state.user_name:
            ...
            if name:
                st.session_state.user_name = name; st.rerun()
        else:
            st.markdown(f"""
            <div style='display:flex;align-items:center;gap:.7rem;
                ...
                <div>
                    <div style='color:#f0f2f7;font-weight:700;font-size:.9rem;'>{st.session_state.user_name}</div>
        ...
        for icon, key, label in [
            ("🏠","dashboard","Dashboard"), ("🗓️","itinerary","Trip Planner"),
            ("🌍","recommendations","Discover"), ("🧰","toolbox","Travel Toolbox"),
            ("🤖","chatbot","AI Assistant"), ("🎬","video","Trip Video"),
            ("📸","vision","Photo Analyzer"), ("📔","journal","Travel Journal"),
        ]:
        ...
        <div style='color:#4b5563;font-size:.72rem;text-align:center;padding:.5rem 0;'>
            Cultural AI Travel v2.0<br>Powered by {GEMINI_MODEL}
        </div>""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
def page_dashboard():
    fname = (st.session_state.user_name or "Traveler").split()[0]
    c1, c2 = st.columns([3, 1])
    with c1:
        st.markdown(f"""
        <div style='margin-bottom:1.5rem;'>
            <div style='font-family:"Playfair Display",serif;font-size:2.5rem;
                        font-weight:800;color:#f0f2f7;line-height:1.1;'>Hello, {fname}! 🌏</div>
    ...
    with c2:
        if st.button("✈️  Plan New Trip", use_container_width=True):
            st.session_state.view = "itinerary"; st.rerun()

    s1, s2, s3, s4 = st.columns(4)
    with s1: st.metric("Destinations", len(DESTINATIONS), "Curated")
    with s2: st.metric("Saved Trips", len(st.session_state.saved_trips), "Your Plans")
    with s3: st.metric("AI Tools", "8", "Available")
    with s4:
        today = datetime.now()
    ...
    cols = st.columns(4)
    for col, (icon, title, sub, view) in zip(cols, [
    ...
    for col, dest in zip(st.columns(3), DESTINATIONS[:3]):
        with col:
            ...
            <div class='dest-card'>
                <div style='position:relative;overflow:hidden;height:180px;'>
                    ...
                    <div style='position:absolute;bottom:10px;right:10px;
                                ...
                                padding:3px 8px;font-size:.78rem;color:#facc15;'>⭐ {dest["rating"]}</div>
                </div>
    ...
    p1, p2 = st.columns(2)
    for i, dest in enumerate(DESTINATIONS[3:7]):
...
def page_itinerary():
    ...
    with st.container():
        ...
        with c1: dest     = st.text_input("🌍 Destination (leave empty for AI pick)", placeholder="e.g. Kyoto, Japan")
        with c2: duration = st.number_input("📅 Days", min_value=1, max_value=30, value=5)
        with c3: age      = st.number_input("👤 Your Age", min_value=10, max_value=99, value=30)
        c4, c5, c6 = st.columns(3)
        with c4: budget = st.selectbox("💰 Budget", ["Budget","Mid-range","Luxury"])
        with c5: season = st.selectbox("🌤️ Season", ["Spring","Summer","Autumn","Winter"])
        with c6: lang   = st.selectbox("🌐 Language", ["English"] + list(LANG_MAP.keys()))

        st.markdown("**🎯 Interests**")
        interest_opts = ["Culture","Nature","Adventure","History","Art","Cuisine","Architecture","Wellness"]
        sel_interests = []
        for i, opt in enumerate(st.columns(8)):
            with opt:
                if st.checkbox(interest_opts[i], key=f"int_{interest_opts[i]}",
                               value=interest_opts[i] in ["Culture","Cuisine"]):
                    sel_interests.append(interest_opts[i])
    ...
    if gen_btn:
        ...
        prefs    = {"destination": target, "age": age, "interests": sel_interests,
                    "duration": duration, "budget": budget, "season": season, "accessibility": access}
        temp_val = random.randint(15, 32)
        cond_val = random.choice(["sunny","partly cloudy","clear skies","light breeze"])

        with st.spinner(f"🧠 {GEMINI_MODEL} is crafting your perfect itinerary…"):
    ...
    if st.session_state.itinerary:
        ...
        if st.session_state.itinerary_image and PIL_OK:
            st.image(Image.open(io.BytesIO(st.session_state.itinerary_image)),
                     use_container_width=True, caption=f"📸 {target}")
        else:
            match = next((d for d in DESTINATIONS if d["name"].lower() in target.lower()), None)
            if match:
                st.markdown(f"<img src='{match['img']}' style='width:100%;border-radius:16px;max-height:300px;object-fit:cover;margin-bottom:1rem;'/>",
                            unsafe_allow_html=True)

        st.markdown("---")
        ctrl1, ctrl2, ctrl3 = st.columns([2, 1, 1])
        with ctrl1: st.markdown(f"### 🗓️ {target} — {len(plan)}-Day Itinerary")
        with ctrl2:
            if FPDF_OK:
        ...
        with st.expander("✏️ Refine Your Itinerary"):
            ...
            with ref_col2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("✨ Refine", use_container_width=True) and ref_req:
                    with st.spinner("Refining…"):
                        updated = ai_refine_itinerary(plan, ref_req)
                    if updated:
                        st.session_state.itinerary = updated; st.success("Itinerary refined!"); st.rerun()
            ...
            <div class='day-card'>
                <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.6rem;'>
                    <div>
                        ...
                        <div style='font-weight:700;color:#f0f2f7;font-size:1.1rem;margin-top:.3rem;'>{day.get("title","")}</div>
                    </div>
                ...
                <div style='color:#d1d5db;line-height:1.65;font-size:.9rem;'>{day.get("description","")}</div>
            </div>""", unsafe_allow_html=True)
...
def page_recommendations():
    ...
    with fc2: cont_filter = st.selectbox("Continent", ["All","Europe","Asia","Africa","South America","North America","Oceania"])
    with fc3: type_filter = st.selectbox("Type", ["All","Historical","Cultural","Nature","Scenic","Adventure"])
    with fc4: unesco_only = st.checkbox("UNESCO only")

    results = DESTINATIONS
    if search:
        q = search.lower()
        results = [d for d in results if any(q in d[k].lower() for k in ["name","country","continent","type"])]
    if cont_filter != "All": results = [d for d in results if d["continent"] == cont_filter]
    if type_filter != "All": results = [d for d in results if d["type"] == type_filter]
    if unesco_only:          results = [d for d in results if d["unesco"]]
    ...
    for i, dest in enumerate(results):
        with cols[i % 3]:
            ...
            if fav_key not in st.session_state: st.session_state[fav_key] = False
            st.markdown(f"""
            <div class='dest-card' style='margin-bottom:1.25rem;'>
                <div style='position:relative;overflow:hidden;height:180px;'>
                    <img src='{dest["img"]}' style='width:100%;height:100%;object-fit:cover;'/>
                    <div style='position:absolute;top:10px;left:10px;'>{badge}</div>
                    <div style='position:absolute;bottom:10px;right:10px;background:rgba(0,0,0,.65);
                                border-radius:8px;padding:3px 9px;color:#facc15;font-size:.82rem;'>
                        ⭐ {dest["rating"]}
                    </div>
                </div>
                <div style='padding:1rem 1rem .75rem;'>
                    ...
                    <div style='color:#6b7280;font-size:.78rem;margin-top:.5rem;'>{dest["visitors"]}M annual visitors</div>
                </div>
            </div>""", unsafe_allow_html=True)
            ...
            with bc1:
                if st.button("Plan ✈️", key=f"plan_{dest['id']}", use_container_width=True):
                    st.session_state.view = "itinerary"; st.rerun()
            with bc2:
                if st.button("❤️" if st.session_state[fav_key] else "🤍",
                             key=f"fav_btn_{dest['id']}", use_container_width=True):
                    st.session_state[fav_key] = not st.session_state[fav_key]; st.rerun()
            with bc3:
                if st.button("Chat 🤖", key=f"chat_{dest['id']}", use_container_width=True):
                    st.session_state.view = "chatbot"
                    st.session_state.chat.append({
                        "id": f"auto_{dest['id']}", "sender": "user",
                        "text": f"Tell me about visiting {dest['name']} in {dest['country']}."})
                    st.rerun()
...
def page_toolbox():
    ...
    with st.container():
        ...
        with tc1: tb_dest   = st.text_input("🌍 Destination", placeholder="e.g. Tokyo, Japan", key="tb_dest")
        with tc2: tb_days   = st.number_input("📅 Days", 1, 60, 5, key="tb_days")
        with tc3: tb_budget = st.selectbox("💰 Budget", ["Budget","Mid-range","Luxury"], key="tb_budget")
        with tc4: tb_season = st.selectbox("🌤️ Season", ["Spring","Summer","Autumn","Winter"], key="tb_season")
        tb_ints = st.multiselect("🎯 Interests",
                                 ["Culture","Cuisine","Adventure","Nature","Art","History","Shopping","Wellness"],
                                 default=["Culture","Cuisine"], key="tb_ints")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if gen_tb and tb_dest:
        with st.spinner(f"🔧 {GEMINI_MODEL} generating toolbox data…"):
            st.session_state.toolbox = {
                "packing":    ai_packing_list(tb_dest, tb_days, tb_season, tb_ints),
                "etiquette":  ai_etiquette(tb_dest),
                "budget":     ai_budget(tb_dest, tb_days, tb_budget),
                "phrasebook": ai_phrasebook(tb_dest),
                "safety":     ai_safety(tb_dest),
                "gems":       ai_hidden_gems(tb_dest),
            }
    ...
    with tabs[0]:
        ...
        if packing:
            ...
            for i, item in enumerate(packing):
                with (pc1 if i < mid else pc2):
                    chk_key = f"pck_{i}_{item[:10]}"
                    if chk_key not in st.session_state: st.session_state[chk_key] = False
                    st.checkbox(item, key=chk_key)
            st.download_button("📋 Export List", "\n".join(f"☐ {x}" for x in packing),
                               f"packing_{dest_label.replace(' ','_')}.txt", "text/plain")
        else:
            st.info("Generate toolbox data to see your smart packing list.")

    with tabs[1]:
        et = tb.get("etiquette")
        md_render(et) if et else st.info("Generate toolbox data to see local etiquette guide.")
    with tabs[2]:
        bud = tb.get("budget")
        md_render(bud) if bud else st.info("Generate toolbox data to see budget estimate.")
    with tabs[3]:
        ph = tb.get("phrasebook")
        md_render(ph) if ph else st.info("Generate toolbox data to see travel phrasebook.")
    with tabs[4]:
        sf = tb.get("safety")
        md_render(sf) if sf else st.info("Generate toolbox data to see safety tips.")
    with tabs[5]:
        gm = tb.get("gems")
        md_render(gm) if gm else st.info("Generate toolbox data to see hidden gems.")
        ...
        with vt2:
            ...
            if result:
                ...
                <div class='ai-card' style='text-align:center;padding:1.5rem;'>
                    <div style='color:#6b7280;font-size:.8rem;margin-bottom:.5rem;'>{lang_sel} Translation</div>
                    <div style='font-size:1.4rem;font-weight:700;color:#f0f2f7;line-height:1.4;'>{result}</div>
                </div>""", unsafe_allow_html=True)
                if GTTS_OK:
                    if st.button("🔊 Play Audio", use_container_width=True):
                        with st.spinner("Generating audio…"):
                            audio = make_tts(result, st.session_state.get("trans_lang","en"))
    ...
    with tabs[7]:
        ...
        if up_img and st.button("🔍 Analyze Image", use_container_width=True):
            ...
            with ia1: st.image(img_bytes, caption="Uploaded Image", use_container_width=True)
            with ia2:
                with st.spinner("Analyzing…"):
                    analysis = ai_analyze_image(img_bytes, up_img.type, img_prompt)
                st.markdown(f"""
...
def page_chatbot():
    ...
    st.markdown(f"<div class='section-sub'>Powered by {GEMINI_MODEL} · Ask anything about destinations, culture, etiquette, and travel planning.</div>",
                unsafe_allow_html=True)

    chat_html = '<div style="display:flex;flex-direction:column;gap:.5rem;padding:1rem 0;">'
    for msg in st.session_state.chat:
        if msg["sender"] == "user":
            chat_html += f"<div class='chat-user'><div class='bubble-user'>{msg['text']}</div></div>"
        else:
            escaped = msg["text"].replace("<","&lt;").replace(">","&gt;")
            chat_html += f"""
            <div class='chat-bot'>
    ...
    with inp_col:
        user_msg = st.text_input("Message Aria", label_visibility="collapsed",
                                 placeholder="Ask about destinations, culture, tips…", key="chat_input")
    with btn_col:
    ...
    if send and user_msg.strip():
        ...
        if GTTS_OK:
            audio = make_tts(reply)
            if audio: st.audio(audio, format="audio/mp3")
        st.rerun()

    st.markdown("<div style='margin-top:.75rem;'><span style='color:#6b7280;font-size:.82rem;'>Quick prompts: </span></div>",
                unsafe_allow_html=True)
    for col, q in zip(st.columns(4),
                      ["Best time to visit Japan?","Cultural etiquette in India?",
                       "Hidden gems in Europe?","Budget travel tips for SE Asia"]):
        with col:
...
# ═══════════════════════════════════════════════════════════════
#  PAGE: TRIP VIDEO (Veo 2)
# ═══════════════════════════════════════════════════════════════
def page_video():
    st.markdown("<div class='section-title'>🎬 Trip Memory Video</div>", unsafe_allow_html=True)
    st.markdown("<div class='section-sub'>Generate a cinematic video memory of your journey using Google's Veo 2 AI</div>", unsafe_allow_html=True)
    st.warning("**⚠️ Paid API Required:** Video generation uses Veo 2, which requires a paid Google Cloud project with Veo API access.")

    with st.container():
        st.markdown("<div class='ai-card'>", unsafe_allow_html=True)
        vid_prompt = st.text_area("Describe your trip memory",
                                  placeholder="e.g. A cinematic video of a serene temple in Kyoto at sunrise…",
                                  height=120)
        v1, v2, v3 = st.columns(3)
        with v1: st.selectbox("Duration", ["5s","8s","10s"])
        with v2: st.selectbox("Aspect Ratio", ["16:9","9:16","1:1"])
        with v3: vid_style = st.selectbox("Style", ["Cinematic","Documentary","Aerial","Time-lapse"])
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.button("🎬 Generate Video"):
        if not vid_prompt.strip():
            ...
            with st.spinner("🎬 Generating video (this may take 2-5 minutes)…"):
                video_url = ai_generate_video(f"{vid_prompt}. Style: {vid_style}. Professional quality travel video.")
            if video_url:
                st.success("✅ Video generated!"); st.video(video_url)
            else:
    ...
    for tip in [
        ...
        if st.button(f"💡 {tip[:70]}…", key=f"vid_tip_{tip[:15]}", use_container_width=True):
...
def page_vision():
    ...
    st.markdown(f"<div class='section-sub'>Powered by {GEMINI_MODEL} — upload any travel photo for deep cultural insights</div>",
                unsafe_allow_html=True)

    up = st.file_uploader("Upload a travel photo", type=["jpg","jpeg","png","webp"])
    if up:
        col1, col2 = st.columns(2)
        with col1: st.image(up, caption="Your Travel Photo", use_container_width=True)
        with col2:
            preset_map = {
                "Cultural History":    "What is the cultural and historical significance here? Include era, civilization, and traditions.",
                "Travel Tips":         "Practical tips for visiting: best time, etiquette, what to wear, hidden spots nearby.",
                "Art & Architecture":  "Analyze architectural style, artistic elements, and design philosophy. Include historical period.",
                "Food & Cuisine":      "Describe cuisine, key dishes, local ingredients, and dining etiquette visible.",
                "Nature & Environment":"Describe the natural landscape, ecosystem, geographical features, and best activities.",
            }
            preset        = st.selectbox("Analysis Type", list(preset_map.keys()))
            custom_prompt = st.text_area("Custom Question (optional)",
                                          placeholder="Or type your own question…", height=80)
            final_prompt  = custom_prompt.strip() or preset_map[preset]
            if st.button("🔍 Analyze Photo", use_container_width=True):
                with st.spinner("🧠 Analyzing…"):
                    st.session_state["vision_result"] = ai_analyze_image(up.read(), up.type, final_prompt)

        if st.session_state.get("vision_result"):
            st.markdown("### 🧠 AI Analysis")
            md_render(st.session_state["vision_result"])
            if GTTS_OK and st.button("🔊 Listen to Analysis"):
                with st.spinner("Generating audio…"):
...
def page_journal():
    ...
    with j1:
        ...
        jnotes = st.text_area("✏️ Your raw notes",
                              placeholder="e.g. Visited temple at sunrise. Monks chanting. Green tea ceremony.",
                              height=200)
    ...
    with j2:
        ...
        if result:
            st.markdown(f"""
            <div style='background:#1c1f28;border:1px solid #2a2f3d;border-radius:20px;padding:1.5rem;'>
                <div style='font-family:"Playfair Display",serif;font-size:1.3rem;
                            font-weight:700;color:#d4f870;margin-bottom:.3rem;'>{jdest_res}</div>
                <div style='color:#6b7280;font-size:.8rem;margin-bottom:1rem;
                            border-bottom:1px solid #2a2f3d;padding-bottom:.75rem;'>✍️ {jdate_res}</div>
                <div style='color:#d1d5db;font-size:.93rem;line-height:1.85;font-style:italic;'>
                    {result.replace(chr(10),"<br>")}
                </div>
            </div>""", unsafe_allow_html=True)
            ec1, ec2 = st.columns(2)
            with ec1:
                st.download_button("📋 Save as Text",
                                   f"# {jdest_res}\n*{jdate_res}*\n\n{result}",
            ...
            <div style='background:#1c1f28;border:1px dashed #2a2f3d;border-radius:20px;
                        padding:3rem;text-align:center;min-height:320px;
                        display:flex;flex-direction:column;justify-content:center;align-items:center;'>
                <div style='font-size:3rem;margin-bottom:1rem;opacity:.3;'>📔</div>
                <div style='color:#6b7280;'>Your journal entry will appear here</div>
            </div>""", unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state.user_name:
        ...
        <div style='max-width:420px;margin:4rem auto;text-align:center;
            ...
            <div style='font-family:"Playfair Display",serif;font-size:2.8rem;
                        font-weight:800;color:#d4f870;margin-bottom:.5rem;'>Cultural AI</div>
    ...
    }.get(st.session_state.view, page_dashboard)()
