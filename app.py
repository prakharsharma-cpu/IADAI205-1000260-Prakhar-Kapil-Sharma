...
def ai_chatbot(message: str, history: list) -> str:
    client = get_client()
    if not client:
        return "⚙️ Please enter your Gemini API key in the sidebar to use Aria."
...
def ai_generate_video(prompt: str) -> str | None:
    """Generate a trip video using Veo (requires paid API key). Returns video URL or None."""
    client = get_client()
    if not client:
        return None
    import time
...
def sidebar():
    with st.sidebar:
        ...
        # API Key
        with st.expander("⚙️ API Configuration", expanded=not get_client()):
            key_in = st.text_input("Gemini API Key", type="password",
        ...
        for icon, key, label in pages:
            ...
            bg = "#d4f870" if active else "transparent"
            color = "#111318" if active else "#9ca3af"
            bdr = "none" if active else "1px solid transparent"
...
def page_itinerary():
    ...
    if gen_btn:
        ...
        if plan:
            # Try image generation, fall back to Unsplash
            img_bytes = None
            ...
            st.error("Could not generate itinerary. Check your API key.")
...
def page_recommendations():
    ...
    for i, dest in enumerate(results):
        with cols[i % 3]:
            ...
            fav_icon = "❤️" if st.session_state[fav_key] else "🤍"
        ...
        if packing:
            ...
            for i, item in enumerate(packing):
                ...
                with col:
                    ...
                    packed = st.checkbox(item, key=chk_key)
            # Export
            if packing:
    ...
    **⚠️ Paid API Required:** Video generation uses the Veo model which requires a paid Google Cloud
    ...
    if st.button("🎬 Generate Video", use_container_width=False):
        if not vid_prompt.strip():
            ...
            st.error("Configure your Gemini API key first.")
        else:
