  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'recommendations':
        return <SmartRecommendations searchQuery={searchQuery} />;
      case 'video':
      ...
      <div className="h-screen bg-[#1a1c23] flex items-center justify-center p-8">
        <div className="max-w-md w-full bg-[#242731] rounded-3xl p-10 border border-[#2a2d39] text-center shadow-2xl">
          ...
          <button 
            ...
            className={`w-full py-4 bg-[#d4f870] hover:bg-[#c2e65d] text-[#1a1c23] rounded-2xl font-bold transition-all flex items-center justify-center space-x-3 shadow-lg shadow-[#d4f870]/10 ${isSigningIn ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {isSigningIn ? (
              <div className="w-5 h-5 border-2 border-[#1a1c23] border-t-transparent rounded-full animate-spin"></div>
            ) : (
              <LogIn size={20} />
            )}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between px-8 py-6">
          <form 
            onSubmit={(e) => {
              e.preventDefault();
              if (searchQuery.trim()) setCurrentView('recommendations');
            }}
